ALTER TABLE "invoices" ADD COLUMN "payment_schedule" jsonb;--> statement-breakpoint
ALTER TABLE "quotes" ADD COLUMN "payment_schedule" jsonb;--> statement-breakpoint

-- ════════════════════════════════════════════════════════════════════
-- Carry the quote's payment-term snapshot onto the generated invoice.
-- Re-declares generate_invoice (from 0012) with payment_schedule added to the
-- invoice INSERT — everything else is identical.
-- ════════════════════════════════════════════════════════════════════
