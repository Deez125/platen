-- ════════════════════════════════════════════════════════════════════
-- Payment-term defaults — extends seed_tenant_defaults() to also seed the
-- starter payment terms (Paid in full · 50% deposit / balance on pickup ·
-- Net 30) shown at the top of the pricing-rules page.
--
-- Apply AFTER drizzle/0015_cuddly_obadiah_stane.sql (creates the table + RLS).
-- Idempotent — safe to re-run. Re-declares seed_tenant_defaults with all the
-- existing sections PLUS the payment-terms section, then backfills existing orgs.
-- ════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- ── Payment terms ───────────────────────────────────────────────────
  IF NOT EXISTS (SELECT 1 FROM payment_term_options WHERE tenant_id = p_org_id) THEN
    INSERT INTO payment_term_options (tenant_id, name, is_default, installments, sort_order)
    VALUES
      (p_org_id, 'Paid in full', true,
        '[{"id":"pif-1","label":"Full payment","mode":"percent","value":100,"trigger":"at_order","netDays":30}]'::jsonb, 1),
      (p_org_id, '50% deposit / balance on pickup', false,
        '[{"id":"ds-1","label":"Deposit","mode":"percent","value":50,"trigger":"at_order","netDays":30},{"id":"ds-2","label":"Balance","mode":"percent","value":50,"trigger":"on_delivery","netDays":30}]'::jsonb, 2),
      (p_org_id, 'Net 30', false,
        '[{"id":"n30-1","label":"Full payment","mode":"percent","value":100,"trigger":"net_days","netDays":30}]'::jsonb, 3);
  END IF;
END;
$$;

-- ── Backfill: any existing org missing payment terms ───────────────────
DO $$
DECLARE r RECORD;
BEGIN
  FOR r IN
    SELECT o.id FROM organizations o
    WHERE NOT EXISTS (SELECT 1 FROM payment_term_options p WHERE p.tenant_id = o.id)
  LOOP
